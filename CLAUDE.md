...
Requires `argp-standalone` from Homebrew (`brew install argp-standalone`) — macOS's libc
doesn't ship `argp.h`/`libargp`, so the Makefile resolves its prefix via
`brew --prefix argp-standalone` for both header and library search paths.
...
- `greep/main.c` — entry point. Parses args via argp (`options.h`/`options.c`), then for
  each input file: spawns a thread (`threaded_find`) that mmaps the file read-only and
...
- `greep/options.c` / `options.h` — argp-based CLI parsing. `arguments_t` holds parsed
  state (search word, verbose flag, file list). Default file list is `/dev/stdin`.
